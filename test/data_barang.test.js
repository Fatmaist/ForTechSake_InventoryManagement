...
const app = require("../app_databarang");
...
describe("Data Barang API Endpoints", () => {
  ...
  it("should update a data barang by ID", async () => {
    const res = await request(app).put(`/api/data_barang/${createdBarangId}`).send({
      nama_barang: "Huawei P30",
      stok: 60,
      id_kategori: 101,
      id_supplier: 991,
    ...
    expect(res.body.data.nama_barang).toEqual("Huawei P30");
  ...
  it("should delete a data barang by ID", async () => {
    const res = await request(app).delete(`/api/data_barang/${createdBarangId}`);
    expect(res.statusCode).toEqual(200);
