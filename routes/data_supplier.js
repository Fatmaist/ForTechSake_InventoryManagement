 ...
 * tags:
 *   name: Data Supplier
 *   description: API for managing data supplier
 */
 ...
 *         content:
 *           application/json:
 *             example: {"status": 200, "message": "Success: Data supplier exist", "data": [{"id_supplier": 1, "nama_supplier": "Supplier A", "no_telepon": 123456789, "alamat": "Address A"}]}
 */
router.get('/data_supplier', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM data_supplier');
    res.status(200).json({ status: 200, message: 'Success: Data supplier exist', data: result.rows });
  } catch (error) {
    console.error(error);
 ...
 */
router.get('/data_supplier/:id', async (req, res) => {
  const { id } = req.params;
  try {
    ...
    if (result.rows.length === 0) {
      ...
      res.status(200).json({ status: 200, message: 'Success: Data supplier exist', data: result.rows[0] });
    }
  } catch (error) {
    console.error(error);
    res.status(500).json({ status: 500, message: 'Internal Server Error' });
 ...
 */
router.post('/data_supplier', async (req, res) => {
 ...
 *             example: {"status": 400, "message": "Failed: Bad Request"}
 */
router.put('/data_supplier/:id', async (req, res) => {
  const { id } = req.params;
 ...
 */
router.delete('/data_supplier/:id', async (req, res) => {
  const { id } = req.params;
  try {
